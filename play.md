...
- EssentialAction is a function:
It is a function that takes a `RequestHeader` and returns an `Accumulator[ByteString, Result]`.

- It is also a `Handler`:
This makes it one of the fundamental request-processing components in Play.

`RequestHeader => Accumulator[ByteString, Result]`
means:
`Given the request metadata, return something that will consume the body and eventually produce a result.`
...
```scala
EssentialAction { rh =>
  Accumulator.done(Results.Ok("Hello"))
}

```

It wraps your function as an `EssentialAction`.
