...
// NOTE: when using REACT_APP_API_URL at local development - after login with SSO it will
...
export const REACT_TROUBLESHOOT_API_PROD_URL =
    process.env.REACT_TROUBLESHOOT_API_PROD_URL ?? `https://192.168.8.4/api`;

export const MAP_SERVER_URL = IN_DEVELOPMENT_MODE
    ? `https://s3-us-west-2.amazonaws.com/${process.env.REACT_APP_MAP_URL}/updated-map`
    : '/static/map';

// get SSO flag from global config loaded in index.html before UI app started
export const USE_SSO = _.get(window, 'Platform_Config.use_oauth', false);

export const isSSOLogin = () => Cookies.get('apiToken') ?? localStorage.getItem('apiToken');

export const isSSOEnabled = () => _.get(window, 'Platform_Config.use_oauth', false);

export const setSSO = (value) => _.set(window, 'Platform_Config.use_oauth', value);

export const setShowJWTTokenInfo = (value) =>
    _.set(window, 'Platform_Config.show_jwt_token_info', value);

export const shouldShowJWTTokenInfo = () =>
    _.get(window, 'Platform_Config.show_jwt_token_info', false);
...
export const TITLE = 'Anywhere';



export function getProviderEndpoint(providerUUID) {
    const customerUUID = localStorage.getItem('customerId');
    // in development mode, we are using http://localhost:9000/api/v1 as the base URL
    // or /api/v1 in production mode
    // http://localhost:9000/api/v1/customers/123/providers/456 for localhost
    // /api/v1/customers/123/providers/456 for production
...
export function getCustomerEndpoint() {
    ...
    // in development mode, we are using http://localhost:9000/api/v1 as the base URL
    // or /api/v1 in production mode
    // http://localhost:9000/api/v1/customers/123 for localhost
    // /api/v1/customers/123 for production
    return `${ROOT_URL}/customers/${customerUUID}`;
}

export function getUniverseEndpoint(universeUUID) {
    const baseUrl = getCustomerEndpoint();
     // in development mode, we are using http://localhost:9000 as the base URL
     // '' for production mode
    // http://localhost:9000/universes/456 for localhost
    // /universes/456 for production
...
export function getTablesEndpoint(universeUUID, tableUUID) {
    ...
    // in development mode, we are using http://localhost:9000 as the base URL
    // '' for production mode
    // http://localhost:9000/tables/123 for localhost
    // /tables/123 for production
